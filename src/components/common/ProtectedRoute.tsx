...
import { ADMIN_MODULES, getAdminModuleById, getAdminModulePermission } from '../../config/adminModules';
...
const ProtectedRoute = ({ allowedRoles, module, children }: ProtectedRouteProps) => {
  ...
  const requestedModule = module ? getAdminModuleById(module) : undefined;
  if (module && (!requestedModule || requestedModule.available === false)) {
    return <Navigate to="/admin" replace state={{ unavailableModule: module }} />;
  }

  if (module && !hasPermission(module, 'view')) {
    const firstAccessible = ADMIN_MODULES.find((item) => item.available !== false && hasPermission(getAdminModulePermission(item), 'view'));
