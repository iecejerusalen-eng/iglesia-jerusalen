...
const SongLyricsEditor = ({ content, onChange, disabled = false }: Props) => {
  ...
  return (
    <div className="border border-gray-300 rounded-lg overflow-hidden bg-white shadow-sm focus-within:border-gold focus-within:ring-1 focus-within:ring-gold transition-all">
      ...
      {!disabled && (
        <div className="flex flex-wrap gap-1 p-2 border-b bg-gray-50 border-gray-200 items-center">
          {/* Undo/Redo */}
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().undo().run(); }}
            onMouseDown={(e) => e.preventDefault()}
            disabled={!editor.can().undo()}
            className="p-1.5 rounded text-gray-600 hover:bg-gray-200 disabled:opacity-30 cursor-pointer" type="button" title="Deshacer">
            <Undo2 size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().redo().run(); }}
            onMouseDown={(e) => e.preventDefault()}
            disabled={!editor.can().redo()}
            className="p-1.5 rounded text-gray-600 hover:bg-gray-200 disabled:opacity-30 cursor-pointer" type="button" title="Rehacer">
            <Redo2 size={16} />
          </button>
          ...
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleHeading({ level: 1 }).run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('heading', { level: 1 }) ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
            type="button" title="Título (Nombre canción)">
            <Heading1 size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleHeading({ level: 2 }).run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('heading', { level: 2 }) ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
            type="button" title="Sección (Verso, Coro)">
            <Heading2 size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleHeading({ level: 3 }).run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('heading', { level: 3 }) ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
          ...
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleBold().run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('bold') ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
            type="button" title="Negrita">
            <Bold size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleItalic().run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('italic') ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
          ...
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().setTextAlign('left').run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive({ textAlign: 'left' }) ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
            type="button" title="Izquierda">
            <AlignLeft size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().setTextAlign('center').run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive({ textAlign: 'center' }) ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
            type="button" title="Centrar">
            <AlignCenter size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().setTextAlign('right').run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive({ textAlign: 'right' }) ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
          ...
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleBulletList().run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('bulletList') ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
            type="button" title="Lista">
            <List size={16} />
          </button>
          <button onClick={(e) => { e.preventDefault(); editor.chain().focus().toggleOrderedList().run(); }}
            onMouseDown={(e) => e.preventDefault()}
            className={`p-1.5 rounded text-gray-600 cursor-pointer ${editor.isActive('orderedList') ? 'bg-gray-200 ring-1 ring-gray-300' : 'hover:bg-gray-200'}`}
          ...
          {/* Typography selector */}
          <div className="flex items-center gap-1 bg-white border border-gray-200 rounded px-1 py-0.5 mr-2">
            <span className="text-[9px] text-gray-400 font-bold px-1 uppercase tracking-wider">Tipografía</span>
            <button
              onClick={(e) => { e.preventDefault(); setFontFamily('sans'); }}
              onMouseDown={(e) => e.preventDefault()}
              className={`px-1.5 py-0.5 rounded text-[10px] font-bold transition-all cursor-pointer ${
                fontFamily === 'sans' ? 'bg-amber-100 text-amber-800' : 'text-gray-500 hover:bg-gray-100'
              }`}
              type="button"
            ...
            <button
              ...
              className={`px-1.5 py-0.5 rounded text-[10px] font-serif font-bold transition-all cursor-pointer ${
                fontFamily === 'serif' ? 'bg-amber-100 text-amber-800' : 'text-gray-500 hover:bg-gray-100'
              }`}
              type="button"
            ...
            <button
              ...
              className={`px-1.5 py-0.5 rounded text-[10px] font-mono font-bold transition-all cursor-pointer ${
                fontFamily === 'mono' ? 'bg-amber-100 text-amber-800' : 'text-gray-500 hover:bg-gray-100'
              }`}
              type="button"
          ...
          <div className="relative">
            ...
            {showChordPicker && (
              <div className="absolute top-full left-0 mt-1 bg-white border border-gray-200 rounded-xl shadow-xl p-3 z-50 w-72">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-bold text-gray-500 uppercase tracking-wider">Seleccionar Acorde</span>
                ...
                <div className="grid grid-cols-7 gap-1 mb-2">
                  {CHORD_PRESETS.map((chord) => (
                    <button
                      ...
                      onMouseDown={(e) => e.preventDefault()}
                      className="px-1 py-1.5 text-xs font-mono font-bold rounded bg-gray-50 hover:bg-red-50 hover:text-red-700 border border-gray-200 hover:border-red-300 transition-all cursor-pointer"
                      type="button"
                    >
                      {chord}
                    </button>
                ...
                <div className="flex gap-1 mt-2">
                  <input
                    ...
                    className="flex-1 text-xs border border-gray-300 rounded px-2 py-1.5 focus:border-amber-400 focus:ring-1 focus:ring-amber-400 outline-none"
                  />
                  <button
                ...
                {editor.isActive('chord') && (
                  <button
                    ...
                    onMouseDown={(e) => e.preventDefault()}
                    className="w-full mt-2 px-3 py-1.5 bg-red-50 text-red-600 text-xs font-semibold rounded hover:bg-red-100 border border-red-200 cursor-pointer transition-colors"
                    type="button"
                  >
          ...
          {/* Colors */}
          <div className="flex items-center gap-0.5 ml-auto bg-white border border-gray-200 rounded px-1.5 py-0.5">
      ...
      {/* Editor Content */}
      <div className={`p-4 bg-white text-gray-800 ProseMirror-wrapper font-${fontFamily}`}>
