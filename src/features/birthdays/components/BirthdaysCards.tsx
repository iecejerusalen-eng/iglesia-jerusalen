...
interface BirthdaysCardsProps { birthdays: BirthdayInfo[]; onCelebrate: (name: string) => void; onMessage?: (birthday: BirthdayInfo) => void; }

export function BirthdaysCards({ birthdays, onCelebrate, onMessage }: BirthdaysCardsProps) {
        ...
        return (
          <article key={item.member.id} className={`group relative overflow-hidden rounded-[1.75rem] border bg-white/75 p-5 shadow-[0_18px_55px_-38px_rgba(15,23,42,0.5)] backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:shadow-xl dark:bg-slate-900/70 ${item.isToday ? 'border-church-gold/60 ring-1 ring-church-gold/20' : 'border-white/80 hover:border-church-gold/35 dark:border-white/10'}`}>
            ...
            <div className="relative flex items-start justify-between gap-3">
              <BirthdayAvatar item={item} sizeClassName="h-20 w-20 border-2 border-white shadow-lg dark:border-slate-700" shapeClassName="rounded-2xl" textClassName="text-xl" />
              <span className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider ${item.isToday ? 'bg-accent-red text-white' : 'bg-primary/8 text-primary dark:bg-white/10 dark:text-slate-200'}`}>
                {item.isToday && <Sparkles size={11} />}{getBirthdayStatusLabel(item)}
              </span>
            </div>
            ...
            <div className="mt-5 flex gap-2">
              <button type="button" onClick={() => onCelebrate(fullName)} className={`inline-flex flex-1 items-center justify-center gap-2 rounded-2xl px-3 py-3 text-xs font-bold transition ${item.isToday ? 'bg-primary text-white shadow-lg shadow-primary/15 hover:bg-primary-dark' : 'bg-slate-100 text-slate-700 hover:bg-church-gold/15 hover:text-church-gold-dark dark:bg-slate-800 dark:text-slate-200'}`}>
                <Gift size={16} /> Celebrar
              </button>
              {onMessage && <button type="button" onClick={() => onMessage(item)} className="inline-flex items-center justify-center rounded-2xl border border-emerald-500/25 bg-emerald-500/10 px-3 text-emerald-700 transition hover:bg-emerald-500/20 dark:text-emerald-300" aria-label={`Enviar felicitación a ${fullName}`} title="Enviar felicitación"><MessageCircle size={16} /></button>}
            </div>
          </article>
        );
      })}
