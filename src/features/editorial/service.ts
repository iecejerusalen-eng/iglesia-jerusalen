...
export const isEditorialSchemaMissing = (error: RpcErrorLike | null): boolean =>
  Boolean(
    ...
    (error.code === "42883" ||
      error.code === "PGRST202" ||
      error.code === "PGRST205" ||
      error.message?.includes("editorial")),
  );

const clientKey = "jerusalem-editorial-client-id";
const tokenKey = (documentId: string): string =>
  `jerusalem-editorial-token:${documentId}`;
const viewSessionKey = "jerusalem-editorial-view-session";
...
export function getEditorialViewSession(): string {
  if (typeof window === "undefined") return crypto.randomUUID();
  const existing = window.localStorage.getItem(viewSessionKey);
  if (existing) return existing;
  const created = crypto.randomUUID();
  window.localStorage.setItem(viewSessionKey, created);
  return created;
}

export async function trackEditorialDocumentView(
  documentId: string,
): Promise<boolean> {
  const { data, error } = await supabase.rpc("track_editorial_document_view", {
    p_document_id: documentId,
    p_session_id: getEditorialViewSession(),
  });
  if (error) throw error;
  return data === true;
}

export async function fetchEditorialSpace(
  slug: string,
): Promise<EditorialSpaceFeed | null> {
  const { data, error } = await supabase.rpc("get_editorial_space", {
    p_slug: slug,
  });
  if (error) throw error;
...
export async function fetchEditorialDocument(
  spaceSlug: string,
  documentId: string,
): Promise<EditorialDocumentResponse | null> {
...
export async function unlockEditorialDocument(
  documentId: string,
  password: string,
): Promise<"success" | "incorrect" | "rate_limited" | "invalid"> {
  ...
  const result = data as {
    success?: boolean;
    reason?: "incorrect" | "rate_limited" | "invalid";
    token?: string;
  } | null;
