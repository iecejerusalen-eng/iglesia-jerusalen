  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [servicesResult, sessionsResult] = await Promise.all([
        ...
        supabase.from('live_service_sessions').select('id,service_id,status,title,stream_url,current_item_id,live_summary,content_blocks').in('status', ['scheduled', 'live']).order('created_at', { ascending: false }).limit(1).maybeSingle(),
      ]);
      if (!mounted) return;
      ...
      if (loadedSession) {
        ...
        const [pollsResult, questionsResult] = await Promise.all([
        ...
        if (pollsResult.error || questionsResult.error) {
          const error = pollsResult.error ?? questionsResult.error;
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      {loading ? <div className="flex justify-center py-20"><Loader2 className="animate-spin text-amber-600" /></div> : <section className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-slate-900 sm:p-7">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <label className="block"><span className="mb-1.5 block text-xs font-bold uppercase tracking-wide text-slate-500">URL de transmisión HTTPS</span><input value={streamUrl} onChange={(event) => setStreamUrl(event.target.value)} placeholder="https://www.youtube.com/watch?v=..." className="min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm dark:border-white/10 dark:bg-slate-950 dark:text-white" /></label>
