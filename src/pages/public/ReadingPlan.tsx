...
const ReadingPlan = () => {
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center py-40 bg-gray-50/50">
        <RefreshCw className="animate-spin text-amber-800" size={32} />
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50/20 to-white py-12 px-4">
      <div className="max-w-5xl mx-auto space-y-10">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-150 p-6 md:p-8 shadow-sm space-y-4 relative overflow-hidden">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-lg font-serif font-bold text-gray-800">Lectura Congregacional (Meta Colectiva)</h2>
                <p className="text-xs text-gray-500 dark:text-gray-400">Uniendo esfuerzos para escudriñar la Palabra de Dios</p>
              </div>
            </div>
            <div className="text-right">
              <span className="text-2xl font-bold text-amber-700">{congregationalProgress.toLocaleString()}</span>
          ...
          <div className="space-y-2">
            ...
            <div className="flex justify-between text-xs text-gray-400 font-medium">
              <span>0%</span>
              <span>Progreso de la Iglesia: {congregationalPercentage}%</span>
              <span>100%</span>
            </div>
          </div>
        </div>
        ...
        {selectedPlan && (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-150 p-6 shadow-sm space-y-4">
                <h3 className="font-serif font-bold text-gray-850">Planes Activos</h3>
                <div className="space-y-2">
                  {plans.map((p) => (
                    <button
                      key={p.id}
                      ...
                      className={`w-full flex items-center justify-between p-3 rounded-xl border text-left text-sm font-medium transition-all ${
                        selectedPlan.id === p.id
                          ...
                          : 'border-gray-200 hover:bg-gray-50 text-gray-600 dark:text-gray-300'
                      }`}
                    >
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-150 p-6 shadow-sm space-y-5">
                <h3 className="font-serif font-bold text-gray-850">Tu Progreso</h3>

                <div className="flex justify-between items-end">
                  <div className="space-y-1">
                    <span className="text-3xl font-serif font-bold text-gray-800">{checkedChapters.length}</span>
                    <span className="text-gray-400 text-xs block">de {selectedPlan.total_chapters} capítulos leídos</span>
                  </div>
                  <span className="text-lg font-bold text-amber-700">{personalPercentage}%</span>
                </div>
                ...
                <div className="space-y-3 pt-3 border-t border-gray-100">
                  ...
                  <div className="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 border border-gray-150 text-xs">
                    <span className="font-semibold text-gray-700">1er Capítulo</span>
                    <Award size={14} className={checkedChapters.length >= 1 ? 'text-amber-500' : 'text-gray-300'} />
                  </div>
                  <div className="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 border border-gray-150 text-xs">
                    <span className="font-semibold text-gray-700">Mitad del Plan (50%)</span>
                    <Award size={14} className={checkedChapters.length >= Math.ceil(selectedPlan.total_chapters * 0.5) ? 'text-amber-500' : 'text-gray-300'} />
                  </div>
                  <div className="flex items-center justify-between p-2.5 rounded-lg bg-gray-50 border border-gray-150 text-xs">
                    <span className="font-semibold text-gray-700">Plan Completo (100%)</span>
            ...
            {/* Checkbox grid */}
            <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-gray-150 p-6 md:p-8 shadow-sm space-y-6">
              <div>
                <h2 className="text-xl font-serif font-bold text-gray-800">{selectedPlan.title}</h2>
              ...
              <div className="border-t border-gray-100 pt-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {saving && <span className="text-xs text-amber-600 font-medium animate-pulse">Guardando...</span>}
                </div>
                      ...
                      <button
                        ...
                        className={`aspect-square flex flex-col items-center justify-center rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                          isRead
                            ...
                            : 'border-gray-200 text-gray-500 dark:text-gray-400 bg-white dark:bg-slate-900 hover:border-amber-400 hover:bg-amber-50/30'
                        }`}
                        title={`Capítulo ${num}`}
                      >
